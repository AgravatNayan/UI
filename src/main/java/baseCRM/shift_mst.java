 ...
 */
@WebServlet("/shift_master")
public class shift_mst extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try { 
			...
			if(lb_continue)
			{
				...
				{
					String emp_id = request.getParameter("shift_id");
					session.setAttribute("EDIT_SHIFT_ID",emp_id);
					session.setAttribute("SHIFT_UPD_REQ","Y");
					jRES.put("STATUS_CD","0");
					jRES.put("ACTION", "E");
					jRES.put("SHIFT_ID", emp_id);
					
				}else if("DELETE".equalsIgnoreCase(action_id))
				{
					String emp_id = request.getParameter("shift_id");
					jout.put("DELETE_FLAG", GetMenu.GetSubDetailsMenuDelete(jmenu,"HRMS","HR_MANGE","SHIFT_MST") );
					jout.put("SHIFT_ID",emp_id);
					try {
						result=APICall.CallAPI("DELETESHIFT",jout.toString());	
						jRES=new JSONObject(result);
					} catch (Exception e) {
						System.out.println("Delete Shift : "+result);
						result=ApplicationConfig.ErrorResponse.replace("$ERROR_MSG$","Error Calling");
					}
					jRES=new JSONObject(result);
					if(!jRES.has("SHIFT_ID"))
					{
						jRES.put("SHIFT_ID",emp_id);
					}
				...
				}else if("NEW".equalsIgnoreCase(action_id) || "UPDATE".equalsIgnoreCase(action_id))
				{
					if("NEW".equalsIgnoreCase(action_id) )
						jout.put("CREATE_FLAG", GetMenu.GetSubDetailsMenuCreate(jmenu,"HRMS","HR_MANGE","SHIFT_MST") );
					else
						jout.put("UPDATE_FLAG", GetMenu.GetSubDetailsMenuUpdate(jmenu,"HRMS","HR_MANGE","SHIFT_MST") );
					JSONObject jEMP=new JSONObject();
					jEMP.put("COMP_CD",comp_cd);
					jEMP.put("SHIFT_ID",ApplicationConfig.nvl(request.getParameter("shiftId")));
					jEMP.put("SHIFT_NAME", ApplicationConfig.nvl(request.getParameter("shiftname")));
					jEMP.put("SHIFT_START_TIME", GetTimeWithSecound(ApplicationConfig.nvl(request.getParameter("inputStartDt"))));//
					jEMP.put("SHIFT_END_TIME", GetTimeWithSecound(ApplicationConfig.nvl(request.getParameter("inputEndDt"))));
					jEMP.put("SHIFT_STATUS", ApplicationConfig.nvl(request.getParameter("inputStatus")));
					jEMP.put("SHIFT_INACTIVE_DT", ApplicationConfig.nvl(request.getParameter("inputInactiveDt")));
					jEMP.put("REMARKS", ApplicationConfig.nvl(request.getParameter("inputremarks")));
					////
					...
					if("UPDATE".equalsIgnoreCase(action_id)) {
						jout.put("SHIFT_ID", ApplicationConfig.nvl(request.getParameter("shiftId")));
						result=APICall.CallAPI("UPDTAESHIFT",jout.toString());
					}else {
						result=APICall.CallAPI("CREATESHIFT",jout.toString());	
					}
				...
				System.out.println("Shift Master("+action_id+") Request :"+jout.toString()+" Response : "+jRES.toString());

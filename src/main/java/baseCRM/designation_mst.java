 ...
 */
@WebServlet("/designation_master")
public class designation_mst extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try { 
			...
			if(lb_continue)
			{
				...
				{
					String id = request.getParameter("desig_id");
					session.setAttribute("EDIT_DESIGNATION_ID",id);
					session.setAttribute("DESIGNATION_UPD_REQ","Y");
					jRES.put("STATUS_CD","0");
					jRES.put("ACTION", "E");
					jRES.put("DESIGNATION_ID", id);
					
				}else if("DELETE".equalsIgnoreCase(action_id))
				{
					String emp_id = request.getParameter("desig_id");
					jout.put("DELETE_FLAG", GetMenu.GetSubDetailsMenuDelete(jmenu,"HRMS","HR_MANGE","DESDIG_MST") );
					jout.put("DESIG_ID",emp_id);
					try {
						result=APICall.CallAPI("DELETEDESIG",jout.toString());	
						jRES=new JSONObject(result);
					} catch (Exception e) {
						System.out.println("Delete Designation : "+result);
						result=ApplicationConfig.ErrorResponse.replace("$ERROR_MSG$","Error Calling");
					}
					jRES=new JSONObject(result);
					if(!jRES.has("DESIGNATION_ID"))
					{
						jRES.put("DESIGNATION_ID",emp_id);
					}
				...
				}else if("NEW".equalsIgnoreCase(action_id) || "UPDATE".equalsIgnoreCase(action_id))
				{
					if("NEW".equalsIgnoreCase(action_id) )
						jout.put("CREATE_FLAG", GetMenu.GetSubDetailsMenuCreate(jmenu,"HRMS","HR_MANGE","DESDIG_MST") );
					else
						jout.put("UPDATE_FLAG", GetMenu.GetSubDetailsMenuUpdate(jmenu,"HRMS","HR_MANGE","DESDIG_MST") );
					JSONObject jEMP=new JSONObject();
					jEMP.put("COMP_CD",comp_cd);
					jEMP.put("DESIG_ID",ApplicationConfig.nvl(request.getParameter("designationId")));
					jEMP.put("DEPART_ID", ApplicationConfig.nvl(request.getParameter("department")));
					jEMP.put("NAME", ApplicationConfig.nvl(request.getParameter("desginationName")));//
					jEMP.put("INACTIVE_DT", ApplicationConfig.nvl(request.getParameter("inputInactiveDt")));
					...
					if("UPDATE".equalsIgnoreCase(action_id)) {
						jout.put("DESIG_ID", ApplicationConfig.nvl(request.getParameter("designationId")));
						result=APICall.CallAPI("UPDATEDESIG",jout.toString());
					}else {
						result=APICall.CallAPI("CREATEDESIG",jout.toString());	
					}
				...
				System.out.println("Designation Master("+action_id+") Request :"+jout.toString()+" Response : "+jRES.toString());
